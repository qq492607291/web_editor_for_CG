...
class cimage extends cTable {
	...
	function __construct() {
		...
		// unid
		$this->unid = new cField('image', 'image', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('image', 'image', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('image', 'image', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID
		$this->ID = new cField('image', 'image', 'x_ID', 'ID', '`ID`', '`ID`', 201, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->ID->Sortable = TRUE; // Allow sort
		$this->fields['ID'] = &$this->ID;

		// Data
		$this->Data = new cField('image', 'image', 'x_Data', 'Data', '`Data`', '`Data`', 201, -1, FALSE, '`Data`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Data->Sortable = TRUE; // Allow sort
		$this->fields['Data'] = &$this->Data;

		// Zoom
		$this->Zoom = new cField('image', 'image', 'x_Zoom', 'Zoom', '`Zoom`', '`Zoom`', 201, -1, FALSE, '`Zoom`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Zoom->Sortable = TRUE; // Allow sort
		$this->fields['Zoom'] = &$this->Zoom;

		// DATETIME
		$this->DATETIME = new cField('image', 'image', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function LoadListRowValues(&$rs) {
		...
		$this->Zoom->setDbValue($rs->fields('Zoom'));
	...
	function RenderEditRow() {
		...
		$this->Zoom->EditValue = $this->Zoom->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Zoom->Exportable) $Doc->ExportCaption($this->Zoom);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Zoom->Exportable) $Doc->ExportField($this->Zoom);
