...
//
// Table class for task_register
//
class ctask_register extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $User;
	...
	function __construct() {
		...
		$this->TableVar = 'task_register';
		$this->TableName = 'task_register';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`task_register`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('task_register', 'task_register', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('task_register', 'task_register', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('task_register', 'task_register', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// User
		$this->User = new cField('task_register', 'task_register', 'x_User', 'User', '`User`', '`User`', 201, -1, FALSE, '`User`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->User->Sortable = TRUE; // Allow sort
		$this->fields['User'] = &$this->User;

		// TaskName
		$this->TaskName = new cField('task_register', 'task_register', 'x_TaskName', 'TaskName', '`TaskName`', '`TaskName`', 201, -1, FALSE, '`TaskName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->TaskName->Sortable = TRUE; // Allow sort
		$this->fields['TaskName'] = &$this->TaskName;

		// Date
		$this->Date = new cField('task_register', 'task_register', 'x_Date', 'Date', '`Date`', '`Date`', 201, -1, FALSE, '`Date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Date->Sortable = TRUE; // Allow sort
		$this->fields['Date'] = &$this->Date;

		// Target
		$this->Target = new cField('task_register', 'task_register', 'x_Target', 'Target', '`Target`', '`Target`', 201, -1, FALSE, '`Target`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Target->Sortable = TRUE; // Allow sort
		$this->fields['Target'] = &$this->Target;

		// Data
		$this->Data = new cField('task_register', 'task_register', 'x_Data', 'Data', '`Data`', '`Data`', 201, -1, FALSE, '`Data`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Data->Sortable = TRUE; // Allow sort
		$this->fields['Data'] = &$this->Data;

		// Complete
		$this->Complete = new cField('task_register', 'task_register', 'x_Complete', 'Complete', '`Complete`', '`Complete`', 201, -1, FALSE, '`Complete`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Complete->Sortable = TRUE; // Allow sort
		$this->fields['Complete'] = &$this->Complete;

		// DATETIME
		$this->DATETIME = new cField('task_register', 'task_register', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`task_register`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "task_registeredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "task_registeradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("task_registerview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("task_registerview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "task_registeradd.php?" . $this->UrlParm($parm);
		else
			$url = "task_registeradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("task_registeredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("task_registeradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("task_registerdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->Complete->setDbValue($rs->fields('Complete'));
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Complete->Exportable) $Doc->ExportCaption($this->Complete);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Complete->Exportable) $Doc->ExportField($this->Complete);
