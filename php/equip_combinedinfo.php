...
//
// Table class for equip_combined
//
class cequip_combined extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $SuitName;
	...
	function __construct() {
		...
		$this->TableVar = 'equip_combined';
		$this->TableName = 'equip_combined';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`equip_combined`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('equip_combined', 'equip_combined', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('equip_combined', 'equip_combined', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('equip_combined', 'equip_combined', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// SuitName
		$this->SuitName = new cField('equip_combined', 'equip_combined', 'x_SuitName', 'SuitName', '`SuitName`', '`SuitName`', 201, -1, FALSE, '`SuitName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->SuitName->Sortable = TRUE; // Allow sort
		$this->fields['SuitName'] = &$this->SuitName;

		// EquipName
		$this->EquipName = new cField('equip_combined', 'equip_combined', 'x_EquipName', 'EquipName', '`EquipName`', '`EquipName`', 201, -1, FALSE, '`EquipName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->EquipName->Sortable = TRUE; // Allow sort
		$this->fields['EquipName'] = &$this->EquipName;

		// DATETIME
		$this->DATETIME = new cField('equip_combined', 'equip_combined', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`equip_combined`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "equip_combinededit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "equip_combinedadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("equip_combinedview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("equip_combinedview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "equip_combinedadd.php?" . $this->UrlParm($parm);
		else
			$url = "equip_combinedadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("equip_combinededit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("equip_combinedadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("equip_combineddelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->SuitName->setDbValue($rs->fields('SuitName'));
	...
	function RenderEditRow() {
		...
		$this->SuitName->EditValue = $this->SuitName->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->SuitName->Exportable) $Doc->ExportCaption($this->SuitName);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->SuitName->Exportable) $Doc->ExportField($this->SuitName);
