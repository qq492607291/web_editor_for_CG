...
$ext_bountytask_log = NULL;

//
// Table class for ext_bountytask_log
//
class cext_bountytask_log extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $taskMode;
	...
	function __construct() {
		...
		$this->TableVar = 'ext_bountytask_log';
		$this->TableName = 'ext_bountytask_log';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`ext_bountytask_log`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('ext_bountytask_log', 'ext_bountytask_log', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('ext_bountytask_log', 'ext_bountytask_log', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('ext_bountytask_log', 'ext_bountytask_log', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// taskMode
		$this->taskMode = new cField('ext_bountytask_log', 'ext_bountytask_log', 'x_taskMode', 'taskMode', '`taskMode`', '`taskMode`', 201, -1, FALSE, '`taskMode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->taskMode->Sortable = TRUE; // Allow sort
		$this->fields['taskMode'] = &$this->taskMode;

		// fromQQ       NOT NULL
		$this->fromQQ_______NOT_NULL = new cField('ext_bountytask_log', 'ext_bountytask_log', 'x_fromQQ_______NOT_NULL', 'fromQQ       NOT NULL', '`fromQQ       NOT NULL`', '`fromQQ       NOT NULL`', 201, -1, FALSE, '`fromQQ       NOT NULL`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->fromQQ_______NOT_NULL->Sortable = TRUE; // Allow sort
		$this->fields['fromQQ       NOT NULL'] = &$this->fromQQ_______NOT_NULL;

		// taskId
		$this->taskId = new cField('ext_bountytask_log', 'ext_bountytask_log', 'x_taskId', 'taskId', '`taskId`', '`taskId`', 201, -1, FALSE, '`taskId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->taskId->Sortable = TRUE; // Allow sort
		$this->fields['taskId'] = &$this->taskId;

		// TIMESTAMP
		$this->TIMESTAMP = new cField('ext_bountytask_log', 'ext_bountytask_log', 'x_TIMESTAMP', 'TIMESTAMP', '`TIMESTAMP`', '`TIMESTAMP`', 201, -1, FALSE, '`TIMESTAMP`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->TIMESTAMP->Sortable = TRUE; // Allow sort
		$this->fields['TIMESTAMP'] = &$this->TIMESTAMP;

		// DATETIME
		$this->DATETIME = new cField('ext_bountytask_log', 'ext_bountytask_log', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`ext_bountytask_log`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "ext_bountytask_loglist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "ext_bountytask_logedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "ext_bountytask_logadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("ext_bountytask_logview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("ext_bountytask_logview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "ext_bountytask_logadd.php?" . $this->UrlParm($parm);
		else
			$url = "ext_bountytask_logadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("ext_bountytask_logedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("ext_bountytask_logadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("ext_bountytask_logdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->taskMode->setDbValue($rs->fields('taskMode'));
	...
	function RenderEditRow() {
		...
		$this->taskMode->EditValue = $this->taskMode->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->taskMode->Exportable) $Doc->ExportCaption($this->taskMode);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->taskMode->Exportable) $Doc->ExportField($this->taskMode);
