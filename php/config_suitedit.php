...
$config_suit_edit = NULL; // Initialize page object first

class cconfig_suit_edit extends cconfig_suit {
	...
	// Page object name
	var $PageObjName = 'config_suit_edit';
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$loaded) { // Load record based on key
					...
					$this->Page_Terminate("config_suitlist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "config_suitlist.php")
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Add_HP
			$this->Add_HP->SetDbValueDef($rsnew, $this->Add_HP->CurrentValue, "", $this->Add_HP->ReadOnly);

			// Add_MP
			$this->Add_MP->SetDbValueDef($rsnew, $this->Add_MP->CurrentValue, "", $this->Add_MP->ReadOnly);

			// Add_Defense
			$this->Add_Defense->SetDbValueDef($rsnew, $this->Add_Defense->CurrentValue, "", $this->Add_Defense->ReadOnly);

			// Add_Magic
			$this->Add_Magic->SetDbValueDef($rsnew, $this->Add_Magic->CurrentValue, "", $this->Add_Magic->ReadOnly);

			// Add_AD
			$this->Add_AD->SetDbValueDef($rsnew, $this->Add_AD->CurrentValue, "", $this->Add_AD->ReadOnly);

			// Add_AP
			$this->Add_AP->SetDbValueDef($rsnew, $this->Add_AP->CurrentValue, "", $this->Add_AP->ReadOnly);

			// Add_Hit
			$this->Add_Hit->SetDbValueDef($rsnew, $this->Add_Hit->CurrentValue, "", $this->Add_Hit->ReadOnly);

			// Add_Dodge
			$this->Add_Dodge->SetDbValueDef($rsnew, $this->Add_Dodge->CurrentValue, "", $this->Add_Dodge->ReadOnly);

			// Add_Crit
			$this->Add_Crit->SetDbValueDef($rsnew, $this->Add_Crit->CurrentValue, "", $this->Add_Crit->ReadOnly);

			// Add_AbsorbHP
			$this->Add_AbsorbHP->SetDbValueDef($rsnew, $this->Add_AbsorbHP->CurrentValue, "", $this->Add_AbsorbHP->ReadOnly);

			// Add_ADPTV
			$this->Add_ADPTV->SetDbValueDef($rsnew, $this->Add_ADPTV->CurrentValue, "", $this->Add_ADPTV->ReadOnly);

			// Add_ADPTR
			$this->Add_ADPTR->SetDbValueDef($rsnew, $this->Add_ADPTR->CurrentValue, "", $this->Add_ADPTR->ReadOnly);

			// Add_APPTR
			$this->Add_APPTR->SetDbValueDef($rsnew, $this->Add_APPTR->CurrentValue, "", $this->Add_APPTR->ReadOnly);

			// Add_APPTV
			$this->Add_APPTV->SetDbValueDef($rsnew, $this->Add_APPTV->CurrentValue, "", $this->Add_APPTV->ReadOnly);

			// Add_ImmuneDamage
			$this->Add_ImmuneDamage->SetDbValueDef($rsnew, $this->Add_ImmuneDamage->CurrentValue, "", $this->Add_ImmuneDamage->ReadOnly);

			// Special_Type
			$this->Special_Type->SetDbValueDef($rsnew, $this->Special_Type->CurrentValue, "", $this->Special_Type->ReadOnly);

			// Special_Value
			$this->Special_Value->SetDbValueDef($rsnew, $this->Special_Value->CurrentValue, "", $this->Special_Value->ReadOnly);
...
// Create page object
if (!isset($config_suit_edit)) $config_suit_edit = new cconfig_suit_edit();

// Page init
$config_suit_edit->Page_Init();

// Page main
$config_suit_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$config_suit_edit->Page_Render();
?>
...
var CurrentForm = fconfig_suitedit = new ew_Form("fconfig_suitedit", "edit");

// Validate form
fconfig_suitedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->u_id->FldCaption(), $config_suit->u_id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_u_id");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($config_suit->u_id->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_acl_id");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->acl_id->FldCaption(), $config_suit->acl_id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_acl_id");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($config_suit->acl_id->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_Name");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Name->FldCaption(), $config_suit->Name->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_HP");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_HP->FldCaption(), $config_suit->Add_HP->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_MP");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_MP->FldCaption(), $config_suit->Add_MP->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_Defense");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_Defense->FldCaption(), $config_suit->Add_Defense->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_Magic");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_Magic->FldCaption(), $config_suit->Add_Magic->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_AD");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_AD->FldCaption(), $config_suit->Add_AD->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_AP");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_AP->FldCaption(), $config_suit->Add_AP->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_Hit");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_Hit->FldCaption(), $config_suit->Add_Hit->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_Dodge");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_Dodge->FldCaption(), $config_suit->Add_Dodge->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_Crit");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_Crit->FldCaption(), $config_suit->Add_Crit->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_AbsorbHP");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_AbsorbHP->FldCaption(), $config_suit->Add_AbsorbHP->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_ADPTV");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_ADPTV->FldCaption(), $config_suit->Add_ADPTV->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_ADPTR");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_ADPTR->FldCaption(), $config_suit->Add_ADPTR->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_APPTR");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_APPTR->FldCaption(), $config_suit->Add_APPTR->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_APPTV");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_APPTV->FldCaption(), $config_suit->Add_APPTV->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Add_ImmuneDamage");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Add_ImmuneDamage->FldCaption(), $config_suit->Add_ImmuneDamage->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Special_Type");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Special_Type->FldCaption(), $config_suit->Special_Type->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_Special_Value");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->Special_Value->FldCaption(), $config_suit->Special_Value->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_DATETIME");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $config_suit->DATETIME->FldCaption(), $config_suit->DATETIME->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_DATETIME");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($config_suit->DATETIME->FldErrMsg()) ?>");
...
fconfig_suitedit.Form_CustomValidate = 
...
fconfig_suitedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $config_suit_edit->ShowPageHeader(); ?>
<?php
$config_suit_edit->ShowMessage();
?>
<form name="fconfig_suitedit" id="fconfig_suitedit" class="<?php echo $config_suit_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($config_suit_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $config_suit_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="config_suit">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($config_suit_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($config_suit->unid->Visible) { // unid ?>
	<div id="r_unid" class="form-group">
		<label id="elh_config_suit_unid" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->unid->FldCaption() ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->unid->CellAttributes() ?>>
<span id="el_config_suit_unid">
<span<?php echo $config_suit->unid->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $config_suit->unid->EditValue ?></p></span>
</span>
<input type="hidden" data-table="config_suit" data-field="x_unid" name="x_unid" id="x_unid" value="<?php echo ew_HtmlEncode($config_suit->unid->CurrentValue) ?>">
<?php echo $config_suit->unid->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->u_id->Visible) { // u_id ?>
	<div id="r_u_id" class="form-group">
		<label id="elh_config_suit_u_id" for="x_u_id" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->u_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->u_id->CellAttributes() ?>>
<span id="el_config_suit_u_id">
<input type="text" data-table="config_suit" data-field="x_u_id" name="x_u_id" id="x_u_id" size="30" placeholder="<?php echo ew_HtmlEncode($config_suit->u_id->getPlaceHolder()) ?>" value="<?php echo $config_suit->u_id->EditValue ?>"<?php echo $config_suit->u_id->EditAttributes() ?>>
</span>
<?php echo $config_suit->u_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->acl_id->Visible) { // acl_id ?>
	<div id="r_acl_id" class="form-group">
		<label id="elh_config_suit_acl_id" for="x_acl_id" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->acl_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->acl_id->CellAttributes() ?>>
<span id="el_config_suit_acl_id">
<input type="text" data-table="config_suit" data-field="x_acl_id" name="x_acl_id" id="x_acl_id" size="30" placeholder="<?php echo ew_HtmlEncode($config_suit->acl_id->getPlaceHolder()) ?>" value="<?php echo $config_suit->acl_id->EditValue ?>"<?php echo $config_suit->acl_id->EditAttributes() ?>>
</span>
<?php echo $config_suit->acl_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Name->Visible) { // Name ?>
	<div id="r_Name" class="form-group">
		<label id="elh_config_suit_Name" for="x_Name" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Name->CellAttributes() ?>>
<span id="el_config_suit_Name">
<textarea data-table="config_suit" data-field="x_Name" name="x_Name" id="x_Name" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Name->getPlaceHolder()) ?>"<?php echo $config_suit->Name->EditAttributes() ?>><?php echo $config_suit->Name->EditValue ?></textarea>
</span>
<?php echo $config_suit->Name->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_HP->Visible) { // Add_HP ?>
	<div id="r_Add_HP" class="form-group">
		<label id="elh_config_suit_Add_HP" for="x_Add_HP" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_HP->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_HP->CellAttributes() ?>>
<span id="el_config_suit_Add_HP">
<textarea data-table="config_suit" data-field="x_Add_HP" name="x_Add_HP" id="x_Add_HP" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_HP->getPlaceHolder()) ?>"<?php echo $config_suit->Add_HP->EditAttributes() ?>><?php echo $config_suit->Add_HP->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_HP->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_MP->Visible) { // Add_MP ?>
	<div id="r_Add_MP" class="form-group">
		<label id="elh_config_suit_Add_MP" for="x_Add_MP" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_MP->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_MP->CellAttributes() ?>>
<span id="el_config_suit_Add_MP">
<textarea data-table="config_suit" data-field="x_Add_MP" name="x_Add_MP" id="x_Add_MP" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_MP->getPlaceHolder()) ?>"<?php echo $config_suit->Add_MP->EditAttributes() ?>><?php echo $config_suit->Add_MP->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_MP->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_Defense->Visible) { // Add_Defense ?>
	<div id="r_Add_Defense" class="form-group">
		<label id="elh_config_suit_Add_Defense" for="x_Add_Defense" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_Defense->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_Defense->CellAttributes() ?>>
<span id="el_config_suit_Add_Defense">
<textarea data-table="config_suit" data-field="x_Add_Defense" name="x_Add_Defense" id="x_Add_Defense" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_Defense->getPlaceHolder()) ?>"<?php echo $config_suit->Add_Defense->EditAttributes() ?>><?php echo $config_suit->Add_Defense->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_Defense->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_Magic->Visible) { // Add_Magic ?>
	<div id="r_Add_Magic" class="form-group">
		<label id="elh_config_suit_Add_Magic" for="x_Add_Magic" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_Magic->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_Magic->CellAttributes() ?>>
<span id="el_config_suit_Add_Magic">
<textarea data-table="config_suit" data-field="x_Add_Magic" name="x_Add_Magic" id="x_Add_Magic" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_Magic->getPlaceHolder()) ?>"<?php echo $config_suit->Add_Magic->EditAttributes() ?>><?php echo $config_suit->Add_Magic->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_Magic->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_AD->Visible) { // Add_AD ?>
	<div id="r_Add_AD" class="form-group">
		<label id="elh_config_suit_Add_AD" for="x_Add_AD" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_AD->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_AD->CellAttributes() ?>>
<span id="el_config_suit_Add_AD">
<textarea data-table="config_suit" data-field="x_Add_AD" name="x_Add_AD" id="x_Add_AD" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_AD->getPlaceHolder()) ?>"<?php echo $config_suit->Add_AD->EditAttributes() ?>><?php echo $config_suit->Add_AD->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_AD->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_AP->Visible) { // Add_AP ?>
	<div id="r_Add_AP" class="form-group">
		<label id="elh_config_suit_Add_AP" for="x_Add_AP" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_AP->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_AP->CellAttributes() ?>>
<span id="el_config_suit_Add_AP">
<textarea data-table="config_suit" data-field="x_Add_AP" name="x_Add_AP" id="x_Add_AP" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_AP->getPlaceHolder()) ?>"<?php echo $config_suit->Add_AP->EditAttributes() ?>><?php echo $config_suit->Add_AP->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_AP->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_Hit->Visible) { // Add_Hit ?>
	<div id="r_Add_Hit" class="form-group">
		<label id="elh_config_suit_Add_Hit" for="x_Add_Hit" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_Hit->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_Hit->CellAttributes() ?>>
<span id="el_config_suit_Add_Hit">
<textarea data-table="config_suit" data-field="x_Add_Hit" name="x_Add_Hit" id="x_Add_Hit" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_Hit->getPlaceHolder()) ?>"<?php echo $config_suit->Add_Hit->EditAttributes() ?>><?php echo $config_suit->Add_Hit->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_Hit->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_Dodge->Visible) { // Add_Dodge ?>
	<div id="r_Add_Dodge" class="form-group">
		<label id="elh_config_suit_Add_Dodge" for="x_Add_Dodge" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_Dodge->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_Dodge->CellAttributes() ?>>
<span id="el_config_suit_Add_Dodge">
<textarea data-table="config_suit" data-field="x_Add_Dodge" name="x_Add_Dodge" id="x_Add_Dodge" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_Dodge->getPlaceHolder()) ?>"<?php echo $config_suit->Add_Dodge->EditAttributes() ?>><?php echo $config_suit->Add_Dodge->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_Dodge->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_Crit->Visible) { // Add_Crit ?>
	<div id="r_Add_Crit" class="form-group">
		<label id="elh_config_suit_Add_Crit" for="x_Add_Crit" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_Crit->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_Crit->CellAttributes() ?>>
<span id="el_config_suit_Add_Crit">
<textarea data-table="config_suit" data-field="x_Add_Crit" name="x_Add_Crit" id="x_Add_Crit" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_Crit->getPlaceHolder()) ?>"<?php echo $config_suit->Add_Crit->EditAttributes() ?>><?php echo $config_suit->Add_Crit->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_Crit->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_AbsorbHP->Visible) { // Add_AbsorbHP ?>
	<div id="r_Add_AbsorbHP" class="form-group">
		<label id="elh_config_suit_Add_AbsorbHP" for="x_Add_AbsorbHP" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_AbsorbHP->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_AbsorbHP->CellAttributes() ?>>
<span id="el_config_suit_Add_AbsorbHP">
<textarea data-table="config_suit" data-field="x_Add_AbsorbHP" name="x_Add_AbsorbHP" id="x_Add_AbsorbHP" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_AbsorbHP->getPlaceHolder()) ?>"<?php echo $config_suit->Add_AbsorbHP->EditAttributes() ?>><?php echo $config_suit->Add_AbsorbHP->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_AbsorbHP->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_ADPTV->Visible) { // Add_ADPTV ?>
	<div id="r_Add_ADPTV" class="form-group">
		<label id="elh_config_suit_Add_ADPTV" for="x_Add_ADPTV" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_ADPTV->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_ADPTV->CellAttributes() ?>>
<span id="el_config_suit_Add_ADPTV">
<textarea data-table="config_suit" data-field="x_Add_ADPTV" name="x_Add_ADPTV" id="x_Add_ADPTV" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_ADPTV->getPlaceHolder()) ?>"<?php echo $config_suit->Add_ADPTV->EditAttributes() ?>><?php echo $config_suit->Add_ADPTV->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_ADPTV->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_ADPTR->Visible) { // Add_ADPTR ?>
	<div id="r_Add_ADPTR" class="form-group">
		<label id="elh_config_suit_Add_ADPTR" for="x_Add_ADPTR" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_ADPTR->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_ADPTR->CellAttributes() ?>>
<span id="el_config_suit_Add_ADPTR">
<textarea data-table="config_suit" data-field="x_Add_ADPTR" name="x_Add_ADPTR" id="x_Add_ADPTR" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_ADPTR->getPlaceHolder()) ?>"<?php echo $config_suit->Add_ADPTR->EditAttributes() ?>><?php echo $config_suit->Add_ADPTR->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_ADPTR->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_APPTR->Visible) { // Add_APPTR ?>
	<div id="r_Add_APPTR" class="form-group">
		<label id="elh_config_suit_Add_APPTR" for="x_Add_APPTR" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_APPTR->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_APPTR->CellAttributes() ?>>
<span id="el_config_suit_Add_APPTR">
<textarea data-table="config_suit" data-field="x_Add_APPTR" name="x_Add_APPTR" id="x_Add_APPTR" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_APPTR->getPlaceHolder()) ?>"<?php echo $config_suit->Add_APPTR->EditAttributes() ?>><?php echo $config_suit->Add_APPTR->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_APPTR->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_APPTV->Visible) { // Add_APPTV ?>
	<div id="r_Add_APPTV" class="form-group">
		<label id="elh_config_suit_Add_APPTV" for="x_Add_APPTV" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_APPTV->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_APPTV->CellAttributes() ?>>
<span id="el_config_suit_Add_APPTV">
<textarea data-table="config_suit" data-field="x_Add_APPTV" name="x_Add_APPTV" id="x_Add_APPTV" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_APPTV->getPlaceHolder()) ?>"<?php echo $config_suit->Add_APPTV->EditAttributes() ?>><?php echo $config_suit->Add_APPTV->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_APPTV->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Add_ImmuneDamage->Visible) { // Add_ImmuneDamage ?>
	<div id="r_Add_ImmuneDamage" class="form-group">
		<label id="elh_config_suit_Add_ImmuneDamage" for="x_Add_ImmuneDamage" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Add_ImmuneDamage->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Add_ImmuneDamage->CellAttributes() ?>>
<span id="el_config_suit_Add_ImmuneDamage">
<textarea data-table="config_suit" data-field="x_Add_ImmuneDamage" name="x_Add_ImmuneDamage" id="x_Add_ImmuneDamage" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Add_ImmuneDamage->getPlaceHolder()) ?>"<?php echo $config_suit->Add_ImmuneDamage->EditAttributes() ?>><?php echo $config_suit->Add_ImmuneDamage->EditValue ?></textarea>
</span>
<?php echo $config_suit->Add_ImmuneDamage->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Special_Type->Visible) { // Special_Type ?>
	<div id="r_Special_Type" class="form-group">
		<label id="elh_config_suit_Special_Type" for="x_Special_Type" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Special_Type->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Special_Type->CellAttributes() ?>>
<span id="el_config_suit_Special_Type">
<textarea data-table="config_suit" data-field="x_Special_Type" name="x_Special_Type" id="x_Special_Type" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Special_Type->getPlaceHolder()) ?>"<?php echo $config_suit->Special_Type->EditAttributes() ?>><?php echo $config_suit->Special_Type->EditValue ?></textarea>
</span>
<?php echo $config_suit->Special_Type->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->Special_Value->Visible) { // Special_Value ?>
	<div id="r_Special_Value" class="form-group">
		<label id="elh_config_suit_Special_Value" for="x_Special_Value" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->Special_Value->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->Special_Value->CellAttributes() ?>>
<span id="el_config_suit_Special_Value">
<textarea data-table="config_suit" data-field="x_Special_Value" name="x_Special_Value" id="x_Special_Value" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($config_suit->Special_Value->getPlaceHolder()) ?>"<?php echo $config_suit->Special_Value->EditAttributes() ?>><?php echo $config_suit->Special_Value->EditValue ?></textarea>
</span>
<?php echo $config_suit->Special_Value->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($config_suit->DATETIME->Visible) { // DATETIME ?>
	<div id="r_DATETIME" class="form-group">
		<label id="elh_config_suit_DATETIME" for="x_DATETIME" class="<?php echo $config_suit_edit->LeftColumnClass ?>"><?php echo $config_suit->DATETIME->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $config_suit_edit->RightColumnClass ?>"><div<?php echo $config_suit->DATETIME->CellAttributes() ?>>
<span id="el_config_suit_DATETIME">
<input type="text" data-table="config_suit" data-field="x_DATETIME" name="x_DATETIME" id="x_DATETIME" placeholder="<?php echo ew_HtmlEncode($config_suit->DATETIME->getPlaceHolder()) ?>" value="<?php echo $config_suit->DATETIME->EditValue ?>"<?php echo $config_suit->DATETIME->EditAttributes() ?>>
</span>
<?php echo $config_suit->DATETIME->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$config_suit_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $config_suit_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $config_suit_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
fconfig_suitedit.Init();
</script>
<?php
$config_suit_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$config_suit_edit->Page_Terminate();
?>
