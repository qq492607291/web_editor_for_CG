...
//
// Table class for config_union
//
class cconfig_union extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $ID;
	...
	function __construct() {
		...
		$this->TableVar = 'config_union';
		$this->TableName = 'config_union';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`config_union`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('config_union', 'config_union', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('config_union', 'config_union', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('config_union', 'config_union', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID
		$this->ID = new cField('config_union', 'config_union', 'x_ID', 'ID', '`ID`', '`ID`', 201, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->ID->Sortable = TRUE; // Allow sort
		$this->fields['ID'] = &$this->ID;

		// LV
		$this->LV = new cField('config_union', 'config_union', 'x_LV', 'LV', '`LV`', '`LV`', 201, -1, FALSE, '`LV`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->LV->Sortable = TRUE; // Allow sort
		$this->fields['LV'] = &$this->LV;

		// EXP
		$this->EXP = new cField('config_union', 'config_union', 'x_EXP', 'EXP', '`EXP`', '`EXP`', 201, -1, FALSE, '`EXP`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->EXP->Sortable = TRUE; // Allow sort
		$this->fields['EXP'] = &$this->EXP;

		// NeedEXP
		$this->NeedEXP = new cField('config_union', 'config_union', 'x_NeedEXP', 'NeedEXP', '`NeedEXP`', '`NeedEXP`', 201, -1, FALSE, '`NeedEXP`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->NeedEXP->Sortable = TRUE; // Allow sort
		$this->fields['NeedEXP'] = &$this->NeedEXP;

		// CDR
		$this->CDR = new cField('config_union', 'config_union', 'x_CDR', 'CDR', '`CDR`', '`CDR`', 201, -1, FALSE, '`CDR`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->CDR->Sortable = TRUE; // Allow sort
		$this->fields['CDR'] = &$this->CDR;

		// LimitLV
		$this->LimitLV = new cField('config_union', 'config_union', 'x_LimitLV', 'LimitLV', '`LimitLV`', '`LimitLV`', 201, -1, FALSE, '`LimitLV`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->LimitLV->Sortable = TRUE; // Allow sort
		$this->fields['LimitLV'] = &$this->LimitLV;

		// Data
		$this->Data = new cField('config_union', 'config_union', 'x_Data', 'Data', '`Data`', '`Data`', 201, -1, FALSE, '`Data`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Data->Sortable = TRUE; // Allow sort
		$this->fields['Data'] = &$this->Data;

		// DATETIME
		$this->DATETIME = new cField('config_union', 'config_union', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`config_union`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "config_unionedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "config_unionadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("config_unionview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("config_unionview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "config_unionadd.php?" . $this->UrlParm($parm);
		else
			$url = "config_unionadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("config_unionedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("config_unionadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("config_uniondelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->NeedEXP->setDbValue($rs->fields('NeedEXP'));
		$this->CDR->setDbValue($rs->fields('CDR'));
		$this->LimitLV->setDbValue($rs->fields('LimitLV'));
	...
	function RenderEditRow() {
		...
		$this->EXP->EditValue = $this->EXP->CurrentValue;
		...
		$this->NeedEXP->EditValue = $this->NeedEXP->CurrentValue;
		...
		$this->CDR->EditValue = $this->CDR->CurrentValue;
		...
		$this->LimitLV->EditValue = $this->LimitLV->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->NeedEXP->Exportable) $Doc->ExportCaption($this->NeedEXP);
					if ($this->CDR->Exportable) $Doc->ExportCaption($this->CDR);
					if ($this->LimitLV->Exportable) $Doc->ExportCaption($this->LimitLV);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->NeedEXP->Exportable) $Doc->ExportField($this->NeedEXP);
						if ($this->CDR->Exportable) $Doc->ExportField($this->CDR);
						if ($this->LimitLV->Exportable) $Doc->ExportField($this->LimitLV);
