...
//
// Table class for shared_data
//
class cshared_data extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $ID;
	...
	function __construct() {
		...
		$this->TableVar = 'shared_data';
		$this->TableName = 'shared_data';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`shared_data`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('shared_data', 'shared_data', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('shared_data', 'shared_data', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('shared_data', 'shared_data', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID
		$this->ID = new cField('shared_data', 'shared_data', 'x_ID', 'ID', '`ID`', '`ID`', 201, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->ID->Sortable = TRUE; // Allow sort
		$this->fields['ID'] = &$this->ID;

		// SECTION
		$this->SECTION = new cField('shared_data', 'shared_data', 'x_SECTION', 'SECTION', '`SECTION`', '`SECTION`', 201, -1, FALSE, '`SECTION`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->SECTION->Sortable = TRUE; // Allow sort
		$this->fields['SECTION'] = &$this->SECTION;

		// DATA
		$this->DATA = new cField('shared_data', 'shared_data', 'x_DATA', 'DATA', '`DATA`', '`DATA`', 201, -1, FALSE, '`DATA`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->DATA->Sortable = TRUE; // Allow sort
		$this->fields['DATA'] = &$this->DATA;

		// DATETIME
		$this->DATETIME = new cField('shared_data', 'shared_data', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`shared_data`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "shared_dataedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "shared_dataadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("shared_dataview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("shared_dataview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "shared_dataadd.php?" . $this->UrlParm($parm);
		else
			$url = "shared_dataadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("shared_dataedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("shared_dataadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("shared_datadelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->SECTION->Exportable) $Doc->ExportCaption($this->SECTION);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->SECTION->Exportable) $Doc->ExportField($this->SECTION);
