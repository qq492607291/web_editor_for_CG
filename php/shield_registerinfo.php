...
$shield_register = NULL;

//
// Table class for shield_register
//
class cshield_register extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $ID;
	...
	function __construct() {
		...
		$this->TableVar = 'shield_register';
		$this->TableName = 'shield_register';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`shield_register`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('shield_register', 'shield_register', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('shield_register', 'shield_register', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('shield_register', 'shield_register', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID
		$this->ID = new cField('shield_register', 'shield_register', 'x_ID', 'ID', '`ID`', '`ID`', 201, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->ID->Sortable = TRUE; // Allow sort
		$this->fields['ID'] = &$this->ID;

		// ID_Type
		$this->ID_Type = new cField('shield_register', 'shield_register', 'x_ID_Type', 'ID_Type', '`ID_Type`', '`ID_Type`', 201, -1, FALSE, '`ID_Type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->ID_Type->Sortable = TRUE; // Allow sort
		$this->fields['ID_Type'] = &$this->ID_Type;

		// Value
		$this->Value = new cField('shield_register', 'shield_register', 'x_Value', 'Value', '`Value`', '`Value`', 201, -1, FALSE, '`Value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Value->Sortable = TRUE; // Allow sort
		$this->fields['Value'] = &$this->Value;

		// GetTime
		$this->GetTime = new cField('shield_register', 'shield_register', 'x_GetTime', 'GetTime', '`GetTime`', '`GetTime`', 201, -1, FALSE, '`GetTime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->GetTime->Sortable = TRUE; // Allow sort
		$this->fields['GetTime'] = &$this->GetTime;

		// Duration
		$this->Duration = new cField('shield_register', 'shield_register', 'x_Duration', 'Duration', '`Duration`', '`Duration`', 201, -1, FALSE, '`Duration`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Duration->Sortable = TRUE; // Allow sort
		$this->fields['Duration'] = &$this->Duration;

		// Pattern
		$this->Pattern = new cField('shield_register', 'shield_register', 'x_Pattern', 'Pattern', '`Pattern`', '`Pattern`', 201, -1, FALSE, '`Pattern`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Pattern->Sortable = TRUE; // Allow sort
		$this->fields['Pattern'] = &$this->Pattern;

		// DATETIME
		$this->DATETIME = new cField('shield_register', 'shield_register', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`shield_register`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "shield_registerlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "shield_registeredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "shield_registeradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("shield_registerview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("shield_registerview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "shield_registeradd.php?" . $this->UrlParm($parm);
		else
			$url = "shield_registeradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("shield_registeredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("shield_registeradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("shield_registerdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->GetTime->setDbValue($rs->fields('GetTime'));
		$this->Duration->setDbValue($rs->fields('Duration'));
		$this->Pattern->setDbValue($rs->fields('Pattern'));
	...
	function RenderEditRow() {
		...
		$this->GetTime->EditValue = $this->GetTime->CurrentValue;
		...
		$this->Pattern->EditValue = $this->Pattern->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->GetTime->Exportable) $Doc->ExportCaption($this->GetTime);
					if ($this->Duration->Exportable) $Doc->ExportCaption($this->Duration);
					if ($this->Pattern->Exportable) $Doc->ExportCaption($this->Pattern);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->GetTime->Exportable) $Doc->ExportField($this->GetTime);
						if ($this->Duration->Exportable) $Doc->ExportField($this->Duration);
						if ($this->Pattern->Exportable) $Doc->ExportField($this->Pattern);
