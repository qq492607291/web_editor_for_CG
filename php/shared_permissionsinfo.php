...
//
// Table class for shared_permissions
//
class cshared_permissions extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $ID;
	...
	function __construct() {
		...
		$this->TableVar = 'shared_permissions';
		$this->TableName = 'shared_permissions';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`shared_permissions`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('shared_permissions', 'shared_permissions', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('shared_permissions', 'shared_permissions', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('shared_permissions', 'shared_permissions', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ID
		$this->ID = new cField('shared_permissions', 'shared_permissions', 'x_ID', 'ID', '`ID`', '`ID`', 201, -1, FALSE, '`ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->ID->Sortable = TRUE; // Allow sort
		$this->fields['ID'] = &$this->ID;

		// Power
		$this->Power = new cField('shared_permissions', 'shared_permissions', 'x_Power', 'Power', '`Power`', '`Power`', 201, -1, FALSE, '`Power`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Power->Sortable = TRUE; // Allow sort
		$this->fields['Power'] = &$this->Power;

		// DATETIME
		$this->DATETIME = new cField('shared_permissions', 'shared_permissions', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`shared_permissions`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "shared_permissionslist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "shared_permissionsedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "shared_permissionsadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("shared_permissionsview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("shared_permissionsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "shared_permissionsadd.php?" . $this->UrlParm($parm);
		else
			$url = "shared_permissionsadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("shared_permissionsedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("shared_permissionsadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("shared_permissionsdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Power->Exportable) $Doc->ExportCaption($this->Power);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Power->Exportable) $Doc->ExportField($this->Power);
