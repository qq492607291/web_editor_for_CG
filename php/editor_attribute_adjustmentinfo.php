...
$editor_attribute_adjustment = NULL;

//
// Table class for editor_attribute_adjustment
//
class ceditor_attribute_adjustment extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $uId;
	...
	function __construct() {
		...
		$this->TableVar = 'editor_attribute_adjustment';
		$this->TableName = 'editor_attribute_adjustment';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`editor_attribute_adjustment`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('editor_attribute_adjustment', 'editor_attribute_adjustment', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('editor_attribute_adjustment', 'editor_attribute_adjustment', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('editor_attribute_adjustment', 'editor_attribute_adjustment', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// uId
		$this->uId = new cField('editor_attribute_adjustment', 'editor_attribute_adjustment', 'x_uId', 'uId', '`uId`', '`uId`', 201, -1, FALSE, '`uId`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->uId->Sortable = TRUE; // Allow sort
		$this->fields['uId'] = &$this->uId;

		// aName
		$this->aName = new cField('editor_attribute_adjustment', 'editor_attribute_adjustment', 'x_aName', 'aName', '`aName`', '`aName`', 201, -1, FALSE, '`aName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->aName->Sortable = TRUE; // Allow sort
		$this->fields['aName'] = &$this->aName;

		// DATETIME
		$this->DATETIME = new cField('editor_attribute_adjustment', 'editor_attribute_adjustment', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`editor_attribute_adjustment`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "editor_attribute_adjustmentlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "editor_attribute_adjustmentedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "editor_attribute_adjustmentadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("editor_attribute_adjustmentview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("editor_attribute_adjustmentview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "editor_attribute_adjustmentadd.php?" . $this->UrlParm($parm);
		else
			$url = "editor_attribute_adjustmentadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("editor_attribute_adjustmentedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("editor_attribute_adjustmentadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("editor_attribute_adjustmentdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->aName->setDbValue($rs->fields('aName'));
	...
	function RenderEditRow() {
		...
		$this->aName->EditValue = $this->aName->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->aName->Exportable) $Doc->ExportCaption($this->aName);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->aName->Exportable) $Doc->ExportField($this->aName);
