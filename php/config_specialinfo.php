...
//
// Table class for config_special
//
class cconfig_special extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $Name;
	...
	function __construct() {
		...
		$this->TableVar = 'config_special';
		$this->TableName = 'config_special';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`config_special`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('config_special', 'config_special', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('config_special', 'config_special', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('config_special', 'config_special', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Name
		$this->Name = new cField('config_special', 'config_special', 'x_Name', 'Name', '`Name`', '`Name`', 201, -1, FALSE, '`Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Name->Sortable = TRUE; // Allow sort
		$this->fields['Name'] = &$this->Name;

		// Condition
		$this->Condition = new cField('config_special', 'config_special', 'x_Condition', 'Condition', '`Condition`', '`Condition`', 201, -1, FALSE, '`Condition`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Condition->Sortable = TRUE; // Allow sort
		$this->fields['Condition'] = &$this->Condition;

		// Type
		$this->Type = new cField('config_special', 'config_special', 'x_Type', 'Type', '`Type`', '`Type`', 201, -1, FALSE, '`Type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Type->Sortable = TRUE; // Allow sort
		$this->fields['Type'] = &$this->Type;

		// Effect
		$this->Effect = new cField('config_special', 'config_special', 'x_Effect', 'Effect', '`Effect`', '`Effect`', 201, -1, FALSE, '`Effect`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Effect->Sortable = TRUE; // Allow sort
		$this->fields['Effect'] = &$this->Effect;

		// CD
		$this->CD = new cField('config_special', 'config_special', 'x_CD', 'CD', '`CD`', '`CD`', 201, -1, FALSE, '`CD`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->CD->Sortable = TRUE; // Allow sort
		$this->fields['CD'] = &$this->CD;

		// DATETIME
		$this->DATETIME = new cField('config_special', 'config_special', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`config_special`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "config_specialedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "config_specialadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("config_specialview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("config_specialview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "config_specialadd.php?" . $this->UrlParm($parm);
		else
			$url = "config_specialadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("config_specialedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("config_specialadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("config_specialdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->CD->setDbValue($rs->fields('CD'));
	...
	function RenderEditRow() {
		...
		$this->CD->EditValue = $this->CD->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->CD->Exportable) $Doc->ExportCaption($this->CD);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->CD->Exportable) $Doc->ExportField($this->CD);
