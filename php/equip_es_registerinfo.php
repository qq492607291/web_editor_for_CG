...
$equip_es_register = NULL;

//
// Table class for equip_es_register
//
class cequip_es_register extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $Name;
	...
	function __construct() {
		...
		$this->TableVar = 'equip_es_register';
		$this->TableName = 'equip_es_register';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`equip_es_register`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('equip_es_register', 'equip_es_register', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('equip_es_register', 'equip_es_register', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('equip_es_register', 'equip_es_register', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Name
		$this->Name = new cField('equip_es_register', 'equip_es_register', 'x_Name', 'Name', '`Name`', '`Name`', 201, -1, FALSE, '`Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Name->Sortable = TRUE; // Allow sort
		$this->fields['Name'] = &$this->Name;

		// QName
		$this->QName = new cField('equip_es_register', 'equip_es_register', 'x_QName', 'QName', '`QName`', '`QName`', 201, -1, FALSE, '`QName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->QName->Sortable = TRUE; // Allow sort
		$this->fields['QName'] = &$this->QName;

		// LV
		$this->LV = new cField('equip_es_register', 'equip_es_register', 'x_LV', 'LV', '`LV`', '`LV`', 201, -1, FALSE, '`LV`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->LV->Sortable = TRUE; // Allow sort
		$this->fields['LV'] = &$this->LV;

		// DATETIME
		$this->DATETIME = new cField('equip_es_register', 'equip_es_register', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`equip_es_register`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "equip_es_registerlist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "equip_es_registeredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "equip_es_registeradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("equip_es_registerview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("equip_es_registerview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "equip_es_registeradd.php?" . $this->UrlParm($parm);
		else
			$url = "equip_es_registeradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("equip_es_registeredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("equip_es_registeradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("equip_es_registerdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->QName->setDbValue($rs->fields('QName'));
	...
	function RenderEditRow() {
		...
		$this->QName->EditValue = $this->QName->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->QName->Exportable) $Doc->ExportCaption($this->QName);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->QName->Exportable) $Doc->ExportField($this->QName);
