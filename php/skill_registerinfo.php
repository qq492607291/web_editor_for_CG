...
//
// Table class for skill_register
//
class cskill_register extends cTable {
	var $unid;
	var $u_id;
	var $acl_id;
	var $User;
	...
	function __construct() {
		...
		$this->TableVar = 'skill_register';
		$this->TableName = 'skill_register';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`skill_register`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// unid
		$this->unid = new cField('skill_register', 'skill_register', 'x_unid', 'unid', '`unid`', '`unid`', 3, -1, FALSE, '`unid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->unid->Sortable = TRUE; // Allow sort
		$this->unid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['unid'] = &$this->unid;

		// u_id
		$this->u_id = new cField('skill_register', 'skill_register', 'x_u_id', 'u_id', '`u_id`', '`u_id`', 3, -1, FALSE, '`u_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->u_id->Sortable = TRUE; // Allow sort
		$this->u_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['u_id'] = &$this->u_id;

		// acl_id
		$this->acl_id = new cField('skill_register', 'skill_register', 'x_acl_id', 'acl_id', '`acl_id`', '`acl_id`', 3, -1, FALSE, '`acl_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// User
		$this->User = new cField('skill_register', 'skill_register', 'x_User', 'User', '`User`', '`User`', 201, -1, FALSE, '`User`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->User->Sortable = TRUE; // Allow sort
		$this->fields['User'] = &$this->User;

		// SkillName
		$this->SkillName = new cField('skill_register', 'skill_register', 'x_SkillName', 'SkillName', '`SkillName`', '`SkillName`', 201, -1, FALSE, '`SkillName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->SkillName->Sortable = TRUE; // Allow sort
		$this->fields['SkillName'] = &$this->SkillName;

		// Proficiency
		$this->Proficiency = new cField('skill_register', 'skill_register', 'x_Proficiency', 'Proficiency', '`Proficiency`', '`Proficiency`', 201, -1, FALSE, '`Proficiency`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Proficiency->Sortable = TRUE; // Allow sort
		$this->fields['Proficiency'] = &$this->Proficiency;

		// LastUseDate
		$this->LastUseDate = new cField('skill_register', 'skill_register', 'x_LastUseDate', 'LastUseDate', '`LastUseDate`', '`LastUseDate`', 201, -1, FALSE, '`LastUseDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->LastUseDate->Sortable = TRUE; // Allow sort
		$this->fields['LastUseDate'] = &$this->LastUseDate;

		// Occupation
		$this->Occupation = new cField('skill_register', 'skill_register', 'x_Occupation', 'Occupation', '`Occupation`', '`Occupation`', 201, -1, FALSE, '`Occupation`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Occupation->Sortable = TRUE; // Allow sort
		$this->fields['Occupation'] = &$this->Occupation;

		// DATETIME
		$this->DATETIME = new cField('skill_register', 'skill_register', 'x_DATETIME', 'DATETIME', '`DATETIME`', ew_CastDateFieldForLike('`DATETIME`', 0, "DB"), 135, 0, FALSE, '`DATETIME`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`skill_register`";
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "skill_registeredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "skill_registeradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("skill_registerview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("skill_registerview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "skill_registeradd.php?" . $this->UrlParm($parm);
		else
			$url = "skill_registeradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("skill_registeredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("skill_registeradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("skill_registerdelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->SkillName->setDbValue($rs->fields('SkillName'));
		$this->Proficiency->setDbValue($rs->fields('Proficiency'));
	...
	function RenderEditRow() {
		...
		$this->SkillName->EditValue = $this->SkillName->CurrentValue;
		...
		$this->Proficiency->EditValue = $this->Proficiency->CurrentValue;
		...
		$this->LastUseDate->EditValue = $this->LastUseDate->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->SkillName->Exportable) $Doc->ExportCaption($this->SkillName);
					if ($this->Proficiency->Exportable) $Doc->ExportCaption($this->Proficiency);
					if ($this->LastUseDate->Exportable) $Doc->ExportCaption($this->LastUseDate);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->SkillName->Exportable) $Doc->ExportField($this->SkillName);
						if ($this->Proficiency->Exportable) $Doc->ExportField($this->Proficiency);
						if ($this->LastUseDate->Exportable) $Doc->ExportField($this->LastUseDate);
