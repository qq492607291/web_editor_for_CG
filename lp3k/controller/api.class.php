...
class apiController extends appController
{
	...
	public function user_sign_up()
    {
        ...
        if( !$jump )
        {
			...
			if( get_var( "SELECT COUNT(*) FROM `".real_table_name('activecode')."` WHERE `code` = '" . s($code) . "' AND `timeline` > '" . date( "Y-m-d H:i:s " , strtotime("-1day")) . "'" ) < 1  )
			return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ACTVECODE') );	
        } 
		
		$table_user = real_table_name('user');
		
		if( get_var("SELECT COUNT(*) FROM `".$table_user."` WHERE `email` = '" . s( t(v('email')) ) . "'") > 0 )
		...
		$sql = "INSERT INTO `".$table_user."` ( `name` , `pinyin` , `email` , `password` , `timeline` ) VALUES ( " . join( ' , ' , $dsql ) . " )";
    ...
    function user_reset_password()
    {
		...
		$newpass = 'passwd'.substr( md5($uid.time().rand( 1 , 9999 )) , $rnd , 6 );

		$table = real_table_name('user');
		$sql = "UPDATE `".$table."` SET `password` = '". md5($newpass)."' WHERE `id` = '" . intval( $uid ) . "' LIMIT 1";
		run_sql( $sql );
    ...
    function player_info()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( get_user_ext_info_by_id($uid,true,false) );
    }
    ...
    function load_maps()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( get_player_maps_info_by_uid($uid) );
    }
    ...
    function pc_move()
    {
		...
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		$x = intval(v('pos_x'));
		if( $x < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'X坐标' ) );
    	$y = intval(v('pos_y'));
    	if( $y < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'Y坐标' ) );
		return self::send_result( player_move($uid,$x,$y) );
    }
    ...
    function maps_exp()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( get_player_exploration_info_by_uid($uid) );
    }
    ...
    function check_loot()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( get_player_exploration_loot_info_by_uid($uid) );
    }
    ...
    function load_equip()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( get_player_equip_info_by_uid($uid) );
    }
    ...
    function item_cat()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( get_item_category_info() );
    }
    ...
    function show_bank()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( get_player_bank_info_by_uid($uid) );
    }
    ...
    function show_package()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( get_player_package_info_by_uid($uid) );
    }
    ...
    function show_item()
    {
    	$uid = intval(v('uid'));
	if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
	return self::send_result( show_item_info_by_uid($uid) );
    }
    ...
    function show_event()
    {
    	$uid = intval(v('uid'));
	if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
	return self::send_result( show_event_info_by_uid($uid) );
    }
	...
	function rnd_evt()
    {
    	$uid = intval(v('uid'));
		if( $uid < 1 ) return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'UID' ) );
		return self::send_result( debug_rand_event_by_uid($uid) );
    }
    ...
    function rnd_exp()
    {
		...
		$op = rand( 1 , 6 );
		$num = intval(v('num'));
		return self::send_result( player_exp_op_by_uid($uid,$op,$num) );
    }
    ...
    function add_attr()
    {
		...
		$opid = intval(v('attid'));
		if($opid==0){
			// $opid=rand( 1 , 5);	//attr id
			return self::send_error( LR_API_ARGS_ERROR , __('INPUT_CHECK_BAD_ARGS' , 'attid' ) );
		}
		$num = intval(v('num'));
		if($num<1) $num=1;
		elseif ($num>5) {
		...
		return self::send_result( player_attr_op_by_uid($uid,$opid,$num) );
    }
	...
	return self::send_result( player_add_item($uid,$name,$desc) );
    }
	...
	return self::send_result( player_add_event($uid,$name,$desc) );
    }
	...
	*/
	...
	public function user_online()
	{
		...
		$sql = "SELECT `uid` , `last_active` , `device` , `place` FROM `".$table."` WHERE `last_active` > '" . date( "Y-m-d H:i:s" , strtotime("-5 minutes") ) . "'";
		if( !$data = get_data( $sql ) ) return self::send_error( LR_API_DB_EMPTY_RESULT , __( 'API_MESSAGE_EMPTY_RESULT_DATA' ) );
