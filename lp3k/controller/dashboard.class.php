...
class dashboardController extends appController
{
	...
	function index()
	{
		$data['title'] = $data['top_title'] = 'Survival Camp';

		$data['outVar']=array();
		$data['outVar']['email']=v('email');
		// $data['outVar']['password']=v('password');
		//print_r($_SESSION,true)	[token] => 02ll710jnrs2e2vgb6qii03it2 [uid] => 1 [uname] => admin [email] => 492607291@qq.com [level] => 9 
		$body = '';
		$body .= $_SESSION['uname'];
		// $body .= ' ( ' . safe_email($_SESSION['email']) .' ) ';
		// $body .= $_SESSION['dispname'];
		$body .= '! 很高兴又看到你！<br/>';
		$body .= '<br/><br/>'. $_SESSION['dispname'];
		$body .= '<br/><br/>';
		$body .= '<br/><br/>';
		...
		if(is_admin()){
			$tmp_html.=admin_nav_html();
		}
		$tmp_html.=user_nav_html('index');
		
		$tmp_html.='';
		$data['outVar']['page_nav']= $tmp_html;

		return render( $data , 'web' , 'camp' );
	}
